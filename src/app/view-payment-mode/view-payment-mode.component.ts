...
export class ViewPaymentModeComponent implements OnInit {
  ...
  onPaymentModeAdd() {
    if(this.selectedPaymentCategory && this.selectedPaymentMode){
      if(!this.criteriaText.includes(this.selectedPaymentCategory) || !this.criteriaText.includes(this.selectedPaymentMode)) {
        this.getdata.push({paymentMethod: this.selectedPaymentMode, payment: this.selectedPaymentCategory})
      }
    }
  }
