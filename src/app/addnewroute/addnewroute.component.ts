...
"values": ["India", "US", "UK"]
  ...
  ngOnInit(): void {
    ...
    this.cmCriteriaDataDetails.forEach(element => {
      this.criteriaMapDdlOptions.push({name: element.displayName, code: element.fieldName});
  ...
  addCriteriaMap() {
    let criteria = this.criteriaMap.criteria.name+this.criteriaMap.condition.code+this.criteriaMap.val.code;
    //console.log("criteria", criteria, )
    let index = this.criteriaText.indexOf(criteria)
    //validation 1
    if(this.criteriaText.length && index != -1) {
      this.ngxToaster.warning(criteria+" already added, please add different case")
      ...
      if(criteria.includes("=")) {
        ...
        this.criteriaText.forEach(element=> {
          ...
          if(splitText == splitdata) {
            this.ngxToaster.warning(" Please select different criteria for Correspondent")
          } else {
           ...
           // console.log("lshhh", lhs, element);
            ...
            if(lhs == elementLhs) {
              this.ngxToaster.warning("Please delete existing criteria "+element+", then add"+criteria);
            } else {
  ...
  onChange(controlId, event) {
    ...
    switch (controlId) {
      case 'criteria': 
      let selectdCorrespondent = this.cmCriteriaDataDetails.filter(x=> event.code == x.fieldName);
      ...
      operations.forEach(element=> {
        this.criteriaEqualsDdlOptions.push({name:element, code: element=="equal" ? "=" : "!="})
      })
